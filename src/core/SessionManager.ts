...
export class SessionManager {
    ...
    create(host: string, ssh: Session["ssh"], tmuxSession: string, shell?: string, existingId?: string): Session {
        ...
        ssh.on("error", (err) => {
            logger.error({ sessionId: id, error: err.message }, "SSH connection error");
        });
    ...
    remove(id: string): boolean {
        ...
        try {
            ...
            logger.error({ sessionId: id, error: (err as Error).message }, "Error destroying SSH connection");
        }
    ...
    list(): Array<Pick<Session, "id" | "host" | "connectedAt" | "lastActivity" | "tmuxSession" | "shell">> {
