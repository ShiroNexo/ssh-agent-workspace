...
export const backupSchema = z.object({
    ...
    paths: z
        ...
        .min(1)
        .max(100)
        .describe("Absolute remote paths (files or directories) to include in the backup archive"),
    local_dest: z
        .string()
        .min(1)
        .describe("Absolute local path where the backup archive will be saved (must end with .tar.gz)"),
    exclude: z
        .array(z.string())
        .max(20)
        .default([])
        .describe('Glob patterns to exclude (e.g. "*.log", "node_modules")'),
    remove_remote_archive: z
        .boolean()
        .default(true)
        .describe("Delete the temporary archive from the remote host after download"),
});
    ...
    inputSchema: {
        ...
        properties: {
            ...
            remove_remote_archive: {
                type: "boolean",
                description: "Delete remote temp archive after download (default: true)",
                default: true,
            },
        },
...
export async function handleBackup(args: unknown, sessionManager: SessionManager, sshManager: SSHManager) {
    ...
    if (isReadOnlyMode(session.host)) {
        return {
            content: [
                {
                    type: "text" as const,
                    text: `Error: Host '${session.host}' is in read-only mode. Backup is disabled.`,
                },
            ],
            isError: true,
        };
    ...
    const statResult = await sshManager.exec(
        session.ssh,
        `stat -c%s ${q(remoteArchive)} 2>/dev/null || wc -c < ${q(remoteArchive)}`,
        5000,
    );
    ...
    try {
        ...
        logger.error(
            { sessionId: session_id, remoteArchive, localDest: resolvedLocal, error: (err as Error).message },
