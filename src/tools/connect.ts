...
export async function handleConnect(
            ...
            if (!isHostAllowed(proxyJumpAlias)) {
                return {
                    content: [
                        {
                            type: "text" as const,
                            text: `Error: Proxy host '${proxyJumpAlias}' is not in the allowed hosts list`,
                        },
                    ],
                    isError: true,
                };
    ...
    try {
        ...
        logger.warn({ host, error: (err as Error).message }, "Shell detection failed, continuing anyway");
    }
