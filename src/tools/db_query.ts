...
export async function handleDbQuery(
  ...
  if (isReadOnlyMode(session.host)) {
    return { content: [{ type: 'text' as const, text: `Error: Host '${session.host}' is in read-only mode. DB query is disabled.` }], isError: true };
  }
  ...
  try {
    ...
    if (result.code !== 0 && !result.stdout.trim()) {
      return {
        content: [{ type: 'text' as const, text: `Error: Query failed (exit ${result.code}): ${result.stderr || 'unknown error'}` }],
        isError: true,
      };
    }
