...
export async function handleDeploy(
  ...
  if (isReadOnlyMode(session.host)) {
    return { content: [{ type: 'text' as const, text: `Error: Host '${session.host}' is in read-only mode. Deploy is disabled.` }], isError: true };
  }
  ...
  const result: Record<string, unknown> = { session_id, files: report, total: files.length, deployed: report.filter((r) => r.status === 'deployed').length };
