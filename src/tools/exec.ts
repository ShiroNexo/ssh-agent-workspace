...
export const execSchema = z.object({
    ...
    lines: z.number().min(1).max(10000).default(200).describe("Number of output lines to capture"),
});
...
export const execTool = {
    ...
    inputSchema: {
        ...
        properties: {
            ...
            wait_ms: {
                type: "number",
                description: "Minimum milliseconds to wait before starting prompt detection (default: 200)",
                default: 200,
            },
            max_wait_ms: {
                type: "number",
                description: "Maximum total milliseconds to wait for prompt stabilization (default: 10000)",
                default: 10000,
            },
            lines: {
                type: "number",
...
export async function handleExec(args: unknown, sessionManager: SessionManager, tmuxManager: TmuxManager) {
    ...
    try {
        ...
        logger.error({ sessionId: session_id, error: (err as Error).message }, "Failed to send command");
        return {
            content: [
                {
                    type: "text" as const,
    ...
    while (elapsed < max_wait_ms) {
        ...
        if (!captureFailed && outputContainsPrompt(output)) {
            stabilized = true;
            logger.info({ sessionId: session_id, elapsedMs: elapsed }, "Prompt stabilized");
            break;
        }
