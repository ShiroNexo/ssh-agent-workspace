...
export const groupExecTool = {
    ...
    inputSchema: {
        ...
        properties: {
            ...
            max_wait_ms: {
                type: "number",
                description: "Max ms for prompt per session (default: 10000)",
                default: 10000,
            },
...
async function executeOnSession(
    ...
    if (isReadOnlyMode(session.host)) {
        return {
            ...
            host: session.host,
            status: "error",
            error: `Host '${session.host}' is in read-only mode`,
        };
    }
...
export async function handleGroupExec(args: unknown, sessionManager: SessionManager, tmuxManager: TmuxManager) {
    ...
    if (parallel) {
        ...
        for (const id of uniqueIds) {
            results.push(await executeOnSession(id, command, wait_ms, max_wait_ms, lines, sessionManager, tmuxManager));
        }
    }
