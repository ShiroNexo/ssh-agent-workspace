...
export async function handleHealthCheck(args: unknown, sessionManager: SessionManager, sshManager: SSHManager) {
                    ...
                    if (memFree || memAvail) {
                        ...
                        if (result.memory_total_mb) {
                            result.memory_used_mb =
                                (result.memory_total_mb as number) - (result.memory_free_mb as number);
                            result.memory_used_pct =
                                Math.round(
                                    ((result.memory_used_mb as number) / (result.memory_total_mb as number)) * 10000,
                                ) / 100;
                        }
                    }
                }
