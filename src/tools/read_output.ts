...
export const readOutputTool = {
  name: 'read_output',
  description:
    'Capture the latest output from a tmux session pane. Returns recent terminal contents.',
  inputSchema: {
...
export async function handleReadOutput(
  ...
  try {
    const output = await tmuxManager.capturePane(
      session.ssh,
      session.tmuxSession,
      lines
    );
    logger.info(
      { sessionId: session_id, lines, outputLength: output.length },
      'Output captured'
    );
    return {
