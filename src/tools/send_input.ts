...
export async function handleSendInput(args: unknown, sessionManager: SessionManager, tmuxManager: TmuxManager) {
    ...
    try {
        ...
        logger.error({ sessionId: session_id, error: (err as Error).message }, "Failed to send input");
        return {
            content: [
                {
                    type: "text" as const,
