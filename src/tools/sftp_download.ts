...
export const sftpDownloadTool = {
  name: 'sftp_download',
  description:
    'Download a file from a remote host via SFTP. Requires an active session.',
  inputSchema: {
...
export async function handleSftpDownload(
  args: unknown,
  ...
  if (isReadOnlyMode(session.host)) {
    return {
      content: [{ type: 'text' as const, text: `Error: Host '${session.host}' is in read-only mode. SFTP download is disabled.` }],
      isError: true,
    };
  }
