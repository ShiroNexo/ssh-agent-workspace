...
export const sftpListTool = {
    name: "sftp_list",
    description: "List files and directories on a remote host via SFTP. Requires an active session.",
    inputSchema: {
...
export async function handleSftpList(args: unknown, sessionManager: SessionManager, sshManager: SSHManager) {
    if (isReadOnlyMode()) {
