...
export const sftpUploadTool = {
    name: "sftp_upload",
    description: "Upload a local file to a remote host via SFTP. Requires an active session.",
    inputSchema: {
...
export async function handleSftpUpload(args: unknown, sessionManager: SessionManager, sshManager: SSHManager) {
    ...
    if (isReadOnlyMode(session.host)) {
        return {
            content: [
                {
                    type: "text" as const,
                    text: `Error: Host '${session.host}' is in read-only mode. SFTP upload is disabled.`,
                },
            ],
            isError: true,
        };
