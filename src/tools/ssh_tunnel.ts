    ...
    inputSchema: {
        ...
        properties: {
            ...
            type: {
                type: "string",
                description:
                    'Tunnel type. "local" forwards local_port -> remote_host:remote_port. "socks5" starts a SOCKS5 proxy.',
                enum: ["local", "socks5"],
            },
            local_port: {
                type: "number",
            ...
            remote_host: {
                type: "string",
                description:
                    'Target host on the remote side. For "local" type, traffic arriving at local_port is forwarded here.',
            },
            remote_port: {
                type: "number",
                description:
                    'Target port on the remote side. For "local" type, traffic arriving at local_port is forwarded here.',
            },
        },
...
export async function handleTunnelOpen(args: unknown, sessionManager: SessionManager, sshManager: SSHManager) {
    ...
    if (isReadOnlyMode(session.host)) {
        return {
            content: [
                {
                    type: "text" as const,
                    text: `Error: Host '${session.host}' is in read-only mode. Tunnels are disabled.`,
                },
            ],
            isError: true,
        };
    ...
    if (!isHostAllowed(session.host)) {
        return {
            content: [{ type: "text" as const, text: `Error: Host '${session.host}' is not allowed` }],
            isError: true,
        };
    }
    ...
    if (!available) {
        return {
            content: [{ type: "text" as const, text: `Error: Local port ${local_port} is already in use` }],
            isError: true,
        };
    }
    ...
    if (!hostConfig) {
        return {
            content: [{ type: "text" as const, text: `Error: Failed to resolve SSH config for '${session.host}'` }],
            isError: true,
        };
    }
    ...
    if (!sshConfig) {
        return {
            content: [{ type: "text" as const, text: `Error: Failed to build SSH config for '${session.host}'` }],
            isError: true,
        };
    }
    ...
    try {
        ...
        return {
            content: [
                { type: "text" as const, text: `Error: SSH tunnel connection failed: ${(err as Error).message}` },
            ],
            isError: true,
        };
    }
    ...
    server.listen(local_port, "127.0.0.1", () => {
        logger.info(
            { tunnelId, type, localPort: local_port, remoteHost: remote_host, remotePort: remote_port },
            "Tunnel opened",
        );
    });
                ...
                text: JSON.stringify(
                    {
                        ...
                        note:
                            type === "socks5"
                                ? "Use localhost:LOCAL_PORT as SOCKS5 proxy"
                                : `Forwarding localhost:${local_port} -> ${remote_host}:${remote_port}`,
                    },
                    null,
                    2,
                ),
...
export async function handleTunnelClose(args: unknown) {
    ...
    return {
        content: [
            { type: "text" as const, text: JSON.stringify({ tunnel_id: parsed.data.tunnel_id, closed }, null, 2) },
        ],
    };
}
