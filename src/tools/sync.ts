    ...
    inputSchema: {
        ...
        properties: {
            ...
            direction: {
                type: "string",
                description:
                    "upload (local->remote), download (remote->local), or bidirectional (both ways, newest wins)",
                enum: ["upload", "download", "bidirectional"],
                default: "upload",
            },
            dry_run: {
                type: "boolean",
                description: "Preview changes without transferring (default: false)",
                default: false,
            },
...
async function listRemoteRecursive(
    ...
    for (const entry of entries) {
        ...
        if (entry.attrs.isFile) {
            ...
            const children = await listRemoteRecursive(
                sshManager,
                session,
                fullPath,
                maxDepth,
...
export async function handleSync(args: unknown, sessionManager: SessionManager, sshManager: SSHManager) {
    ...
    if (isReadOnlyMode(session.host)) {
        return {
            content: [
                {
                    type: "text" as const,
                    text: `Error: Host '${session.host}' is in read-only mode. Sync is disabled.`,
                },
            ],
            isError: true,
        };
    ...
    for (const relPath of allPaths) {
        ...
        if (!remote && local) {
            ...
            if (!dry_run) {
                try {
                    const remoteFile = remote_path.endsWith("/")
                        ? `${remote_path}${relPath}`
                        : `${remote_path}/${relPath}`;
