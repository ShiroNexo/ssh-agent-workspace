...
export const tailLogSchema = z.object({
  ...
  follow_ms: z.number().min(0).max(30000).default(0).describe('If >0, poll for new lines this many milliseconds and return appended output'),
});
...
export async function handleTailLog(
  args: unknown,
  ...
  try {
    ...
    if (initial.code !== 0 && !initial.stdout) {
      return {
        content: [{ type: 'text' as const, text: `Error: Failed to read '${file_path}': ${initial.stderr || initial.stdout || 'unknown error'}` }],
        isError: true,
      };
    }
        ...
        try {
          ...
          if (next.stdout && next.stdout !== output) {
            ...
            if (next.stdout.length > prevLen && next.stdout.endsWith(output.slice(-200))) {
              ...
              const diff = next.stdout.slice(Math.max(0, next.stdout.length - (next.stdout.length - prevLen + 2000)));
